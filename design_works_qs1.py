...
class Ui_MainWindow(object):
    ...
    def setInput_fft(self):
        if not self.X_lims.text() or not \
            ...
            self.scope_scale_FFT or not \
            self.scope_delay_FFT:
            alert = QMessageBox()
    ...
    def collect_fft_data(self):
        ...
        self.Mainwin.fft_dat.scope_scale = self.scope_scale_FFT.text()
        self.Mainwin.fft_dat.scope_delay = self.scope_delay_FFT.text()
    ...
    def setInput(self):
        if not self.scope_IP.text() or not \
            ...
            self.Ki.text() or not \
            self.err_min or not \
            self.smooth_win or not \
            self.first_correction_ind or not \
            self.flatness_crit or not \
            self.Tos or not \
            self.scope_scale_flat or not \
            self.scope_delay_flat or not \
    ...
    def collect_calib_data(self):
        self.Mainwin.calib_dat.calib_input_data.scope_IP = self.scope_IP.text()
        self.Mainwin.calib_dat.calib_input_data.sparta_IP = self.sparta_IP.text()
