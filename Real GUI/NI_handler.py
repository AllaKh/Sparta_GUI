# This Function samples the Signal From the NI and returns the signal from the NI
# inputs- 
# anlg_ch=["Dev1/ai0"]- Signal NI IO 
# trig_ch=["Dev1/PFI0"]- Signal Trigger
# clk_timing=2e6- NI Clock 
#  samp_per_chan=200000buffer side
# nsamp_per_ch=4000  
# NumOfChirps=2 Chirps, each raw is diffrent Chirp
# plt_flag= debug 
import nidaqmx
import numpy as np
...
def sample_ni (anlg_ch=["Dev1/ai0"],trig_ch=["Dev1/PFI0"],clk_timing=2e6,samp_per_chan=200000,nsamp_per_ch=4000,NumOfChirps=2,plt_flag=0) :  
    ...
    if plt_flag:
        plt.plot(np.transpose(SensorData))
        plt.show()
        #plt.plot(b[1])
        #plt.show()
        #print(b) 
