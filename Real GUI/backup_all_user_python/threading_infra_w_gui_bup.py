...
#     SampledData=np.loadtxt("this_dat.txt")
    ...
    if len(cal_in_dat.lin_dat.iters)==1:
        n_inds_vec=np.linspace(400,800,num=cal_in_dat.lin_dat.iters,endpoint=True)
    else:
        n_inds_vec=cal_in_dat.lin_dat.iters
...
def calib_fun(my_ques,args):
    ...
    cal_out_dat.lin_graph.set_data(ydat=OldLut)
        ...
        if continue_to_next_loop:
            ...
            if but_sum>1:
                ...
                if but_state.lin_but: # request to perform linearization by pressing the linearization button
                    ...
                    print("******* ask alla to implement cal_in_dat.scope_dat.set_scope_dat******")
...
def fft_fun(my_ques,args):
    ...
    while 1:
        try:
            ...
            print("no data in Q_state_fft",my_ques.Q_state_fft.qsize())
            pass
        if state_dat.busy:
            continue
        ...
        try:
            ...
            counter+=1
            print('slept %d' %counter)
            print("no data in Q_fft_input",my_ques.Q_FFT_input.qsize())
            pass
        if first_input_received:
            ...
            fft_out_dat.fft_graph.set_data(xdat=freq,ydat=fftstd,xlim=fft_in_dat.xlim,ylim=fft_in_dat.ylim,ylog=fft_in_dat.ylog)
