...
def irad_stfftLinCalib(sensorTdata,target_freq,plot_flag,LoadFromScope,n_inds_per_slice):
    ...
    #     h=figure
    fft_dat=np.fft.fft( sensorTdata,axis=0)
    cur_mean_fft=(np.mean (np.imag(fft_dat),axis=1)**2+np.mean (np.real(fft_dat),axis=1)**2)**0.5
    try:
        noise_now_pp=np.polyfit(np.log(fvec[std_fit_inds]),np.log(cur_mean_fft[std_fit_inds]),1)# index is intentionally "1"  to discard DC bin. this is not an error in transfer from matlab...
        cur_mean_fft_cor_full=cur_mean_fft[0:len(fvec)]-np.exp(noise_now_pp[1])*fvec**noise_now_pp[0] 

    except:
        print("slice %d has no std" %counter)
        cur_mean_fft_cor_full=cur_mean_fft[0:len(fvec)]
    if plot_flag:    
    ...
    for ind in slice_start_ind_vec :
        ...
        remove_ind= np.mean(mean_cur_Tdat)<0.05
        ...
        std_fit_inds=np.squeeze(np.asarray(np.nonzero(cur_mean_fft[1:len(fvec)]>0))+1)
        try:
            noise_now_pp=np.polyfit(np.log(fvec[std_fit_inds]),np.log(cur_mean_fft[std_fit_inds]),1)# index is intentionally "1"  to discard DC bin. this is not an error in transfer from matlab...
    ...
    return max_bins,max_freqs,max_vals,widths,cur_mean_fft_cor,fvec
