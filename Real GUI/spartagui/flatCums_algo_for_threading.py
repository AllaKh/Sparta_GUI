...
def flattenLaser(signal_dat,list_vco_vec,residual,cums_dat,signal_thold=0.15):
# old: def flattenLaser(signal_dat,list_vco_vec,residual,Vstart,Kp,Ki,Kd,errMin,smooth_win,flat_window,signal_thold=0.15):
...
## parameters of the whole system
    Kp=cums_dat.Kp #0.0001
    Ki=cums_dat.Ki
    Kd=cums_dat.Kd  #0.1 
    Vstart=cums_dat.V0   
    errMin=cums_dat.err_min
    smooth_win=cums_dat.smooth_win
    flat_window=cums_dat.flat_window
    Fs=cums_dat.Fs # sampling frequency of the Msignal (units of Hz). will be probably will be different in the real system
    Tos=cums_dat.Tos #100e-6; # the time delay between ignition and sampling (units of sec)
    ...
    first_correction_ind=cums_dat.first_correction_ind
